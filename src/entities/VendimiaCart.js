...
export class VendimiaCart {
    ...
    _attackCooldown = 0;
    _throwCD = 1.4;
    static SPEED = 21; // persigue, más rápido que el sprint de Canito (18)
    static DETECT_R = 130;
    static TURN_RATE = 1.4;
    static HIT_DAMAGE = 4; // si te embiste
    static HIT_RADIUS = 4.2;
    static HIT_INTERVAL = 1.0;
    static COLLIDER_R = 2.6;
    static THROW_INTERVAL = 0.8; // más rápido que las reinas (1.8)
    static THROW_RANGE_MIN = 6;
    static THROW_RANGE_MAX = 55;
    constructor() {
        ...
        for (const oz of [-0.7, 0.7])
            add(new THREE.TorusGeometry(0.72, 0.06, 6, 16), MAT_HOOP, 0.55, 2.2, -2.4 + oz);
    ...
    tryThrow(target, dt) {
        ...
        const dz = target.z - this.position.z;
        const dist = Math.hypot(dx, dz);
        if (dist < VendimiaCart.THROW_RANGE_MIN || dist > VendimiaCart.THROW_RANGE_MAX)
            return null;
        ...
        origin.x += ux * 2.2;
        origin.z += uz * 2.2;
        const kinds = ['grape', 'grape', 'bottle', 'melon'];
