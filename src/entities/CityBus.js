...
function logoTex() {
    ...
    c.lineWidth = 13;
    c.beginPath();
    c.arc(w / 2, h / 2 - 18, 96, 0, Math.PI * 2);
    c.stroke();
    c.fillStyle = '#ffffff';
    c.textAlign = 'center';
...
export class CityBus {
    ...
    _wps;
    _i = 1;
    static SPEED = 2.6; // bien despacio
    static HALF_LEN = 5.5;
    static HALF_W = 1.45;
    ...
    _build() {
        ...
        for (const sx of [-1, 1])
            this._add(new THREE.BoxGeometry(0.28, 0.2, 0.15), CREAM, sx * 0.85, 1.2, 5.6);
        ...
        for (const sx of [-1, 1])
            this._add(new THREE.BoxGeometry(0.06, 0.06, 9.2), METAL, sx * 1.23, railY, -0.4);
        for (let i = 0; i <= 6; i++)
            for (const sx of [-1, 1])
        ...
        for (const z of [3.6, -3.6])
            for (const sx of [1, -1])
                this._add(wheelGeo, WHEEL, sx * 1.2, 0.55, z);
    ...
    _place() {
        ...
        this.collider.minX = this._x - hx;
        this.collider.maxX = this._x + hx;
        this.collider.minZ = this._z - hz;
        this.collider.maxZ = this._z + hz;
    }
    ...
    update(dt) {
        ...
        if (dist <= step) {
            ...
            this._x += this.dirX * step;
            this._z += this.dirZ * step;
        }
        this._place();
    }
}
