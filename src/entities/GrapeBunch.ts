...
export class GrapeBunch {
  ...
  private _life = 3;
  private _alive = true;
  private _spin = (Math.random() - 0.5) * 3;

  static readonly RADIUS       = 0.6;
  static readonly DAMAGE       = 2;
  static readonly GRAVITY      = 14;   // m/s² — softer arc than a thrown car
  ...
  constructor(origin: THREE.Vector3, vel: THREE.Vector3) {
    ...
    for (const row of rows) {
      for (let i = 0; i < row.n; i++) {
        ...
        m.position.set(
          Math.cos(a) * row.r + (Math.random() - 0.5) * 0.05,
          row.y           + (Math.random() - 0.5) * 0.04,
          Math.sin(a) * row.r + (Math.random() - 0.5) * 0.05,
        );
        m.castShadow = true;
        this.group.add(m);
      }
