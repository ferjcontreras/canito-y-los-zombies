...
export class ZombieGaucho {
  ...
  private _biteTimer = 0;
  private _throwCooldown = 2 + Math.random() * 3;     // initial stagger
  private static readonly BITE_DURATION = 0.32;
  static readonly DANCE_DURATION   = 2.8;
  static readonly STUMBLE_DURATION = 0.9;
  ...
  private _danceCooldown = 1 + Math.random() * 2;
  private _stumbleTimer    = 0;
  private _stumbleCooldown = 1.5 + Math.random() * 2.5;

  static readonly WALK_SPEED       = 3.2;
  static readonly RUN_SPEED        = 18;   // same as Canito sprint — they match pace, can't be outrun
  static readonly THROWER_SPEED    = 2.2;  // throwers are heavy and slow
  static readonly DETECT_R         = 150;
  static readonly ATTACK_R         = 1.7;
  static readonly DAMAGE_INTERVAL  = 1.4;
  static readonly HIT_RADIUS       = 1.4;
  static readonly COLLIDER_R       = 0.45;
  // Thrower-specific
  static readonly THROW_RANGE_MIN  = 15;
  static readonly THROW_RANGE_MAX  = 65;
  static readonly THROW_INTERVAL   = 2.5;
  static readonly THROWER_HP       = 3;
  static readonly BOSS_HP          = 24;   // fireball sponge — needs a long fight
  static readonly CABALLITO_HP     = 24;   // Caballitos de Marly bosses
  static readonly QUEEN_HP         = 3;    // needs 3 fireballs to go down
  static readonly CABALLITO_SPEED  = 5.0;  // aggressive statues — charge faster
  // Caballito spit (ranged attack)
  static readonly SPIT_RANGE_MIN   = 6;
  static readonly SPIT_RANGE_MAX   = 65;
  static readonly SPIT_INTERVAL    = 1.3;
  // Queen-specific: lob grape bunches from a standoff distance instead of biting
  static readonly GRAPE_RANGE_MIN  = 4;
  static readonly GRAPE_RANGE_MAX  = 40;
  ...
  constructor(opts: { runner?: boolean; thrower?: boolean; boss?: boolean; queen?: boolean; caballito?: boolean } = {}) {
    ...
    if (this.isBoss)            this.group.scale.setScalar(2.6);
    else if (this.isCaballito)  this.group.scale.setScalar(1.8);
    else if (this.isThrower)    this.group.scale.setScalar(1.45);
  ...
  private _buildCaballito(): void {
    const m = (geo: THREE.BufferGeometry, mat: THREE.Material, x: number, y: number, z: number, parent: THREE.Object3D = this.group): THREE.Mesh => {
      const me = new THREE.Mesh(geo, mat); me.position.set(x, y, z); parent.add(me); return me;
    };
    ...
    for (const sx of [-1, 1] as const) {
      ...
      if (sx < 0) this._legL = pivot; else this._legR = pivot;
    }
    ...
    armUp.rotation.z = 1.0; armUp.rotation.x = -0.3;
    ...
    for (const ax of [-0.04, 0.04, 0]) m(new THREE.ConeGeometry(0.03, 0.22, 4), MAT_MARBLE2, tx + 0.05 + ax, 2.15, 0.05); // arrows
  }
  ...
  tryThrow(target: THREE.Vector3, dt: number): { origin: THREE.Vector3; vel: THREE.Vector3 } | null {
    if (!this.isThrower) return null;
    ...
    if (dist < ZombieGaucho.THROW_RANGE_MIN || dist > maxRange) return null;
  ...
  tryThrowGrapes(target: THREE.Vector3, dt: number): { origin: THREE.Vector3; vel: THREE.Vector3 } | null {
    if (!this.isQueen) return null;
    this._throwCooldown -= dt;
    if (this._throwCooldown > 0) return null;
    const dx = target.x - this.group.position.x;
    const dz = target.z - this.group.position.z;
    const dist = Math.hypot(dx, dz);
    if (dist < ZombieGaucho.GRAPE_RANGE_MIN || dist > ZombieGaucho.GRAPE_RANGE_MAX) return null;
  ...
  trySpit(target: THREE.Vector3, dt: number): { origin: THREE.Vector3; vel: THREE.Vector3 } | null {
    ...
    const dz = target.z - this.group.position.z;
    const dist = Math.hypot(dx, dz);
    if (dist < ZombieGaucho.SPIT_RANGE_MIN || dist > ZombieGaucho.SPIT_RANGE_MAX) return null;
    ...
    const origin = this.group.position.clone();
    origin.y += 3.2;
    origin.x += ux * 0.6; origin.z += uz * 0.6;
    const vel = new THREE.Vector3(ux * vh, vy, uz * vh);
    return { origin, vel };
  }

  /** Cumbia/chacarera shimmy — big hip sway, hops, spin wiggle, arms up. */
  private _animateDance(dt: number): void {
    this._phase += dt * 9;
  ...
  update(dt: number, target: THREE.Vector3, colliders: AABB[] = []): { hitTarget: boolean } {
    ...
    if (!this.isQueen && dist < ZombieGaucho.ATTACK_R) {
      ...
      if (!holdGround) {
        ...
        if (isWalker) {
          if (this._danceTimer > 0) { this._danceTimer -= dt; dancing = true; }
          ...
          if (stumbling) speedMul = 0.08;              // nearly stops while tripping
        }
