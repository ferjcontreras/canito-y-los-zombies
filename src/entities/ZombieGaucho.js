...
export class ZombieGaucho {
    ...
    _dancingNow = false; // ¿está en su baile ahora mismo? (walkers)
    get isDancing() { return this._dancingNow; }
    _hp;
    ...
    _armR;
    _speed;
    _style = Math.random() * Math.PI * 2;
    _limp = 0.85 + Math.random() * 0.35;
    ...
    static SNACK_RANGE_MIN = 5;
    static SNACK_RANGE_MAX = 38;
    static SNACK_INTERVAL = 1.0;
    constructor(opts = {}) {
    ...
    _blendRotation(obj, x, z, amount, y) {
        if (!obj)
            return;
        obj.rotation.x += (x - obj.rotation.x) * amount;
        if (y !== undefined)
            obj.rotation.y += (y - obj.rotation.y) * amount;
        obj.rotation.z += (z - obj.rotation.z) * amount;
    }
    _poseArms(leftX, rightX, leftZ, rightZ, amount = 0.35) {
    ...
    tryThrowSnack(target, dt) {
        // Sólo los caminantes comunes, y sólo mientras están en su baile.
        if (this.isRunner || this.isThrower || this.isQueen || this.isCaballito || this.isBoss)
            return null;
        if (!this._dancingNow)
            return null;
        this._throwCooldown -= dt;
        ...
        const dz = target.z - this.group.position.z;
        const dist = Math.hypot(dx, dz);
        if (dist < ZombieGaucho.SNACK_RANGE_MIN || dist > ZombieGaucho.SNACK_RANGE_MAX)
            return null;
        ...
        origin.x += ux * 0.4;
        origin.z += uz * 0.4;
        const kind = Math.random() < 0.5 ? 'empanada' : 'mate';
    ...
    _animateDance(dt) {
        ...
        this._poseArms(-1.85 + Math.sin(ph) * 0.75, -1.85 - Math.sin(ph) * 0.75, 0.55 + Math.sin(ph) * 0.35, -0.55 - Math.sin(ph) * 0.35, 0.8);
    }
    ...
    update(dt, target, colliders = []) {
        ...
        if (!this.isQueen && dist < ZombieGaucho.ATTACK_R) {
            ...
            this._poseArms(-1.35 - bitePulse * 0.45 + Math.max(0, claw) * 0.18, -1.35 - bitePulse * 0.45 + Math.max(0, -claw) * 0.18, 0.24 + bitePulse * 0.18, -0.24 - bitePulse * 0.18, 0.42);
            ...
            if (!holdGround) {
                ...
                if (dancing) {
                    ...
                    this._blendRotation(this._head, -0.10 + bitePulse * 0.25 + Math.sin(this._phase * 0.8) * 0.06, Math.sin(this._phase * 0.72 + this._style) * (this._fastGait ? 0.20 : 0.14), 0.25);
                    this.group.position.y = Math.abs(Math.sin(this._phase)) * bobAmt;
                    const reach = this._fastGait ? 1.45 : this.isThrower ? 0.86 : 1.12;
                    this._poseArms(-reach - Math.max(0, -sw) * 0.36 + bitePulse * -0.25, -reach - Math.max(0, sw) * 0.36 + bitePulse * -0.25, 0.24 + Math.sin(this._phase * 0.7 + this._style) * 0.13, -0.24 - Math.sin(this._phase * 0.73 + this._style) * 0.13, 0.28);
                }
            }
            else {
