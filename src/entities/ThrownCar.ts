...
export class ThrownCar {
  ...
  private _life = 2.5;
  private _alive = true;
  private _hitSolid = false;      // smashed into a building/scenery this flight
  private _spin = new THREE.Vector3(
    (Math.random() - 0.5) * 6,
    (Math.random() - 0.5) * 4,
    (Math.random() - 0.5) * 6,
  );

  static readonly RADIUS  = 1.3;
  static readonly DAMAGE  = 6;
  static readonly GRAVITY = 18;     // m/s² — exaggerated for visible arc
  ...
  static readonly BLOCK_HEIGHT = 9;
  ...
  constructor(origin: THREE.Vector3, vel: THREE.Vector3) {
    ...
    for (const [x, z] of [[0.9, 0.58], [0.9, -0.58], [-0.9, 0.58], [-0.9, -0.58]] as Array<[number, number]>) {
  ...
  update(dt: number, colliders: AABB[] = []): void {
    ...
    this._life -= dt;
    if (this._life <= 0 || p.y < 0) this._alive = false;
  }
