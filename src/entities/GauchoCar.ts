...
export class GauchoCar {
  ...
  static readonly SPEED        = 22;     // m/s — más rápido que el sprint de Canito (18)
  static readonly DETECT_R     = 90;
  static readonly TURN_RATE    = 1.6;    // rad/s
  static readonly HIT_DAMAGE   = 3;
  static readonly HIT_RADIUS   = 2.7;
  static readonly HIT_INTERVAL = 0.9;
  static readonly FIRE_HITS    = 3;      // fireballs needed to destroy
  static readonly COLLIDER_R   = 1.8;    // body radius for scenery / house collision
  ...
  constructor() {
    ...
    for (const [x, z] of [[0.92, 1.45], [-0.92, 1.45], [0.92, -1.45], [-0.92, -1.45]] as Array<[number, number]>) {
  ...
  update(dt: number, target: THREE.Vector3, colliders: AABB[] = []): { hitTarget: boolean; blocked: boolean } {
    if (!this.alive) return { hitTarget: false, blocked: false };
