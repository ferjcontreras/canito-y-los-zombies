...
async function main() {
    ...
    // más densidad para que haya acción enseguida.
    {
        ...
        const band = (n, xMin, xMax, halfZ) => {
            ...
            while (spawned < n && tries++ < n * 25) {
                ...
                if (Math.hypot(zx - START_X, zz - START_Z) < 55)
                    continue;
                if (Math.hypot(zx, zz) < 150)
    ...
    const onRoad = (x, z, margin = 2) => {
        for (const e of graph.edges) {
            ...
            if (dx * dx + dz * dz < (e.halfW + margin) * (e.halfW + margin))
                return true;
        }
        return false;
    };
    ...
    const gauchoSnacks = []; // empanadas y mates de los bailarines
    let canitoHP = 20;
    let killCount = 0;
    let barkCD = 0;
    ...
    const showLevelCard = (lvl, subtitle, onDone) => {
    ...
    const checkLevel1Boss = () => {
        if (level1BossActive || gameOver || victory)
            return;
        if (canitoPos.x > CHILE_X + 6)
            return; // recién al llegar a Chile
        ...
        for (let i = gauchoSnacks.length - 1; i >= 0; i--) {
            ...
            if (s.alive) {
                ...
                if (d < GauchoSnack.RADIUS + 0.5 && s.position.y < 2.2) {
                    if (!gameOver)
                        setHP(canitoHP - s.damage, s.position.x, s.position.z);
                    s.kill();
                }
            }
            if (!s.alive) {
