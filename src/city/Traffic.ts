...
interface Vehicle {
  ...
  col: AABB; halfBody: number; scale: number; lastHonk: number; dead?: boolean;
}
...
export class TrafficManager {
  ...
  infectNear(px: number, pz: number, r: number): { x: number; z: number; angle: number } | null {
    const r2 = r * r;
    for (let i = 0; i < this.vehicles.length; i++) {
      ...
      if (dx * dx + dz * dz < r2) {
        ...
        v.col.minX = v.col.maxX = 1e6; v.col.minZ = v.col.maxZ = 1e6;   // collider lejos
  ...
  update(
    ...
    runOver?: (x: number, z: number, moto: boolean) => boolean,
    ...
    for (let iv = 0; iv < n; iv++) {
      ...
      if (hazards) {
        let nd = Infinity;
        for (const h of hazards) { const ddx = h.x - vx, ddz = h.z - vz; const d2 = ddx * ddx + ddz * ddz; if (d2 < nd) nd = d2; }
        panic = nd < PANIC2;
      }
      ...
      if (panic) {
        ...
        if (now - v.lastHonk > 0.7) { v.lastHonk = now; onHonk?.(v.agent.x, v.agent.z, v.moto); }
        if (runOver && runOver(vx, vz, v.moto) && v.moto) { v.dead = true; anyDead = true; }   // la moto se cae
      } else {
        ...
        if (wantHonk && now - v.lastHonk > 1.6) { v.lastHonk = now; onHonk?.(v.agent.x, v.agent.z, v.moto); }
      }
    ...
    if (anyDead) {
      this.vehicles = this.vehicles.filter(v => {
        if (v.dead) { this._scene.remove(v.group); v.col.minX = v.col.maxX = 1e6; v.col.minZ = v.col.maxZ = 1e6; return false; }
        return true;
      });
    }
  }
  ...
  nearest(x: number, z: number): { x: number; z: number; d: number } | null {
    ...
    for (const v of this.vehicles) {
      ...
      if (d < best) { best = d; bx = v.agent.x; bz = v.agent.z; found = true; }
    }
    return found ? { x: bx, z: bz, d: best } : null;
  }

  /** Destruye un vehículo cerca del punto (Canito puede matarlos). */
  killNear(x: number, z: number, r: number): boolean {
    const r2 = r * r;
    for (let i = 0; i < this.vehicles.length; i++) {
      const v = this.vehicles[i];
      ...
      if (dx * dx + dz * dz < r2) {
        this._scene.remove(v.group);
        v.col.minX = v.col.maxX = 1e6; v.col.minZ = v.col.maxZ = 1e6;
        this.vehicles.splice(i, 1);
        return true;
      }
    }
