...
export class CivilianManager {
  ...
  nearest(x: number, z: number): { x: number; z: number; d: number } | null {
    ...
    for (const c of this.civs) {
      if (c.state === 'grabbed') continue;
      const d = Math.hypot(c.x - x, c.z - z);
      if (d < best) { best = d; bx = c.x; bz = c.z; found = true; }
    }
  ...
  update(
    dt: number, zombies: ZRef[], colliders: AABB[], scene: THREE.Scene,
    px = 0, pz = 0, onSave?: (x: number, z: number) => void,
  ): Array<[number, number]> {
