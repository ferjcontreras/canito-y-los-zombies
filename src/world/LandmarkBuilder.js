...
function tube2D(parent, pts, z0, r, mat) {
...
function spiral(parent, cx, cy, z0, r0, r1, turns, mat, tubeR = 0.022) {
    const pts = [];
...
function cCurve(parent, x1, y1, x2, y2, z0, bx, by, tubeR, mat) {
    tube2D(parent, [[x1, y1], [(x1 + x2) / 2 + bx, (y1 + y2) / 2 + by], [x2, y2]], z0, tubeR, mat);
}
// ── Portones sub-builders ─────────────────────────────────────────
function ironPillar(g, x, z0, iron, gold, glass) {
    const b = (w, h, d, mat, oy) => box(g, mat, w, h, d, x, oy, z0);
    const c = (rt, rb, h, mat, oy) => cyl(g, mat, rt, rb, h, 12, x, oy, z0);
...
function gatePanel(g, cx, z0, width, height, iron, gold, isMain) {
...
function fenceSection(g, cx, z0, width, iron) {
...
function mainArch(g, z0, iron, gold) {
    const R = 4.0;
    const baseY = 5.85;
    ...
    for (let i = 1; i < 9; i++) {
        const a = (i / 9) * Math.PI;
        cCurve(g, Math.cos(a) * (R - 0.5), Math.sin(a) * (R - 0.5) + baseY, Math.cos(a) * R, Math.sin(a) * R + baseY, z0, 0, 0, 0.028, iron);
    }
    for (let i = 1; i < 7; i++) {
        const a = (i / 7) * Math.PI;
 ...
 */
export function buildParkWall(scene, cx, zMin, zMax, zGapMin, zGapMax) {
    const wallMat = lam(0x3a3028);
    const capMat = lam(0x4a4038);
    const segs = [[zMin, zGapMin], [zGapMax, zMax]];
    const colliders = [];
 ...
 */
export function buildPortones(scene, cx, cz) {
    ...
    const Z0 = 0;
    for (const px of [-9, -4, 4, 9])
        ironPillar(group, px, Z0, iron, gold, glass);
