...
function halfWidthOf(type: string): number { return HALF_WIDTH[type] ?? 3; }

function asphaltColor(type: string): number {
  if (type.startsWith('motorway'))  return ASPHALT.motorway;
  if (type.startsWith('trunk'))     return ASPHALT.trunk;
  if (type.startsWith('primary'))   return ASPHALT.primary;
  if (type.startsWith('secondary')) return ASPHALT.secondary;
  if (type.startsWith('tertiary'))  return ASPHALT.tertiary;
  return ASPHALT.default;
}

function hasCenterline(type: string): boolean {
...
function buildRibbon(
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (len < 1e-6) {
      if (positions.length >= 6) positions.push(...positions.slice(-6));
      else positions.push(x, y, z, x, y, z);
      continue;
    }
    const nx = -dz / len;
    const nz =  dx / len;
...
function buildRibbonUV(
  pts: [number, number][], halfW: number, offset: number, y: number, tile: number,
): THREE.BufferGeometry | null {
  const n = pts.length;
  if (n < 2) return null;
  const positions: number[] = [];
  ...
  for (let i = 0; i < n; i++) {
    ...
    const v = cum / tile;
    positions.push(cx + nx * halfW, y, cz + nz * halfW); uvs.push(0, v);
    positions.push(cx - nx * halfW, y, cz - nz * halfW); uvs.push((2 * halfW) / tile, v);
  }
  for (let i = 0; i < n - 1; i++) {
    const a = i * 2;
    indices.push(a, a + 2, a + 1, a + 1, a + 2, a + 3);
...
let _cobbleTex: THREE.Texture | null = null;
function cobbleTexture(): THREE.Texture {
  if (_cobbleTex) return _cobbleTex;
  const s = 128;
  const cv = document.createElement('canvas'); cv.width = s; cv.height = s;
  const c = cv.getContext('2d')!;
  c.fillStyle = '#5f5a54'; c.fillRect(0, 0, s, s);   // junta
...
function buildZebra(
  cx: number, cz: number,        // intersection centre
  ux: number, uz: number,        // road direction unit vector (pointing AWAY from intersection)
...
export function buildStreets(
  ...
  const asphaltByColor = new Map<number, THREE.BufferGeometry[]>();
  const sidewalkGeos:   THREE.BufferGeometry[] = [];
  const centerlineGeos: THREE.BufferGeometry[] = [];
  const cobbleGeos:     THREE.BufferGeometry[] = [];

  const zebraPos: number[] = [];
  const zebraIdx: number[] = [];
  ...
  for (const way of ways) {
    ...
    for (const nid of way.nodes) {
      ...
      if (node) pts.push(proj.project(node.lat, node.lon));
    }
    ...
    if (isCobble) {
      ...
      if (cob) cobbleGeos.push(cob);
    } else {
      const asphaltGeo = buildRibbon(pts, hw, 0, 0.04);
      if (!asphaltGeo) continue;
      const color = asphaltColor(type);
      if (!asphaltByColor.has(color)) asphaltByColor.set(color, []);
    ...
    if (hw >= 2.5 && !isOneway) {
      ...
      if (right) sidewalkGeos.push(right);
      if (left)  sidewalkGeos.push(left);
      ...
      if (cR) curbGeos.push(cR);
      if (cL) curbGeos.push(cL);
    }
    ...
    if (hasCenterline(type) && !isCobble) {
      ...
      if (centre) centerlineGeos.push(centre);
    }
    ...
    if (hw >= 3 && pts.length >= 2) {
      ...
      if ((usage.get(way.nodes[0]) ?? 0) >= 2) {
        ...
        if (L > 4) buildZebra(x0, z0, dx / L, dz / L, hw, zebraPos, zebraIdx, 0.07);
      }
      // Last node
      if ((usage.get(way.nodes[n - 1]) ?? 0) >= 2) {
        ...
        const L = Math.hypot(dx, dz);
        if (L > 4) buildZebra(x1, z1, dx / L, dz / L, hw, zebraPos, zebraIdx, 0.07);
      }
    }
  }

  // ── Commit ────────────────────────────────────────────────────────────────
  const commit = (geos: THREE.BufferGeometry[], color: number, y_emit = false) => {
  ...
  for (const [color, geos] of asphaltByColor) commit(geos, color);
  ...
  if (zebraPos.length > 0) {
    ...
    const mesh = new THREE.Mesh(
      geo,
      new THREE.MeshLambertMaterial({ color: CROSSING_COLOR, side: THREE.FrontSide }),
    );
    mesh.receiveShadow = true;
    scene.add(mesh);
  }
