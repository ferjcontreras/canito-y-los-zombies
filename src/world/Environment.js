...
function buildSkyDome(parent) {
    ...
    // Crisp sun disc
    const sun = new THREE.Mesh(new THREE.SphereGeometry(95, 24, 16), new THREE.MeshBasicMaterial({ color: 0xfff0c8, fog: false }));
...
function buildMountainRange(parent, R, amp, segs, lowC, highC, snowFrac, seed, centerAngle, arcHalf) {
    const positions = [];
    const colors = [];
    ...
    const rad = (i) => R * (0.97 + 0.05 * Math.sin(i * 1.7 + seed));
    const topColor = (h) => h > snowH ? snow.clone() : low.clone().lerp(high, Math.min(1, h / amp));
    const pos = (x, y, z) => positions.push(x, y, z);
    const col = (c) => colors.push(c.r, c.g, c.b);
    for (let i = 0; i < segs; i++) {
        const a0 = (i / segs) * Math.PI * 2;
        const a1 = ((i + 1) / segs) * Math.PI * 2;
        // Only emit segments inside the western arc
        if (Math.abs(angleDiff(a0)) > arcHalf && Math.abs(angleDiff(a1)) > arcHalf)
            continue;
...
function buildClouds(parent) {
    ...
    for (let c = 0; c < CLOUDS; c++) {
        ...
        for (let p = 0; p < puffs; p++) {
            ...
            m.position.set((Math.random() - 0.5) * scale * 2.6, (Math.random() - 0.5) * scale * 0.5, (Math.random() - 0.5) * scale * 1.5);
