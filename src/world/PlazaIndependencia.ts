...
interface Jet { mesh: THREE.Mesh; phase: number; maxH: number; baseY: number; }

export interface PlazaParts { colliders: AABB[]; update(dt: number): void; }

export function buildPlazaIndependencia(scene: THREE.Scene, cx: number, cz: number): PlazaParts {
  ...
  const addJet = (x: number, z: number, baseY: number, maxH: number) => {
    const m = new THREE.Mesh(jetGeo, jetMat);
    m.position.set(x, baseY, z);
    scene.add(m);
    jets.push({ mesh: m, phase: Math.random() * Math.PI * 2, maxH, baseY });
  };
  const tri = (pos: number[], a: number[], b: number[], c: number[]) => pos.push(...a, ...b, ...c);
  const meshFrom = (pos: number[], mat: THREE.Material, recv = true): void => {
  ...
  const arc = (a: number, r: number, y: number): number[] => [FX + Math.sin(a) * r, y, FZ + Math.cos(a) * r];
  const N = 36;
  ...
  for (let k = 0; k < 3; k++) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      tri(pos, b0, t0, t1); tri(pos, b0, t1, b1);
    }
  ...
  // ── Fuente chica RECTANGULAR (más al este, detrás de la grande) ────────────
  {
    ...
    rim(W, 0.5, rx, rz - D / 2); rim(W, 0.5, rx, rz + D / 2);
    rim(0.5, D, rx - W / 2, rz); rim(0.5, D, rx + W / 2, rz);
    const water = new THREE.Mesh(new THREE.BoxGeometry(W - 0.5, 0.14, D - 0.5), waterMat);
    water.position.set(rx, 0.32, rz); scene.add(water);
    for (let i = -1; i <= 1; i++) addJet(rx, rz + i * 2.6, 0.34, 1.6 + Math.random());
...
function buildPaths(scene: THREE.Scene, cx: number, cz: number): void {
  const PY = 0.0;
  const seg = (x0: number, z0: number, x1: number, z1: number, hw: number): void => {
    ...
    const m = new THREE.Mesh(g, pathMat); m.receiveShadow = true; scene.add(m);
  };
  const rect = (rcx: number, rcz: number, w: number, d: number): void => {
    const g = new THREE.PlaneGeometry(w, d); g.rotateX(-Math.PI / 2); g.translate(rcx, PY, rcz);
    const m = new THREE.Mesh(g, pathMat); m.receiveShadow = true; scene.add(m);
  };
...
function buildSign(scene: THREE.Scene, signX: number, cz: number, colliders: AABB[]): void {
  ...
  const glyphs: Array<{ rows: string[]; color: number }> = [{ rows: HEART, color: 0xe23b3b }];
  const word = 'MENDOZA';
  for (let i = 0; i < word.length; i++) glyphs.push({ rows: FONT[word[i]], color: grad[i % grad.length] });
  ...
  const base = new THREE.Mesh(
    new THREE.BoxGeometry(2.8, 0.24, span + 1.0),
    new THREE.MeshLambertMaterial({ color: 0x2a2a2e, flatShading: true }),
  );
...
function buildEscudo(scene: THREE.Scene, cx: number, cz: number, colliders: AABB[]): void {
  const g = new THREE.Group();
  g.position.set(cx, 0, cz);
  g.rotation.y = -Math.PI / 2;        // frente (+Z local) → oeste (−X mundo)
  scene.add(g);

  const neon = (c: number, e: number, i = 1.0) =>
    new THREE.MeshLambertMaterial({ color: c, emissive: e, emissiveIntensity: i, flatShading: true });
  ...
  const at = (geo: THREE.BufferGeometry, mat: THREE.Material, x: number, y: number, z: number, rotZ = 0): THREE.Mesh => {
    const m = new THREE.Mesh(geo, mat); m.position.set(x, y, z); m.rotation.z = rotZ; m.castShadow = true; g.add(m); return m;
  };
  ...
  for (const ox of [-3.0, 3.0]) at(new THREE.BoxGeometry(0.22, ocY, 0.22), frameM, ox, ocY / 2, -0.15);

  // Óvalo (relleno azul tenue + aro de neón brillante)
  const fill = at(new THREE.SphereGeometry(1, 22, 18), ovalFill, 0, ocY, 0); fill.scale.set(RX, RY, 0.1);
  const rim  = at(new THREE.TorusGeometry(1, 0.07, 8, 36), ovalRim, 0, ocY, ZF); rim.scale.set(RX, RY, 1);
  ...
  for (const ox of [-0.7, 0, 0.7]) at(new THREE.ConeGeometry(0.55, 0.7, 4), greenM, ox, ocY - 1.25, ZF);
...
function neonText(g: THREE.Group, str: string, lcx: number, ly: number, lz: number, cell: number, mat: THREE.Material): void {
...
function buildPlayground(scene: THREE.Scene, cx: number, cz: number, colliders: AABB[]): void {
  ...
  const at = (geo: THREE.BufferGeometry, mat: THREE.Material, x: number, y: number, z: number): THREE.Mesh => {
    const m = new THREE.Mesh(geo, mat); m.position.set(x, y, z); m.castShadow = true; scene.add(m); return m;
  };

  // Piso de arena
  const floor = at(new THREE.BoxGeometry(20, 0.1, 16), sand, cx, 0.05, cz); floor.castShadow = false; floor.receiveShadow = true;
  ...
  // Tobogán (slide)
  {
    ...
    for (const [ox, oz] of [[-0.6, -0.6], [0.6, -0.6], [-0.6, 0.6], [0.6, 0.6]] as Array<[number, number]>)
    ...
    // escalera
    for (let i = 0; i < 3; i++) at(new THREE.BoxGeometry(1.0, 0.08, 0.2), metal, tx, 0.5 + i * 0.45, tz - 0.8);
  }
  ...
  // Subibaja
  {
    ...
    for (const o of [-2.2, 2.2]) at(new THREE.CylinderGeometry(0.12, 0.12, 0.5, 8), blue, px, 1.1, pz + o);
  }
