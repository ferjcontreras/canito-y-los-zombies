...
function carMatrix(x, z, rotY) {
    const m = new THREE.Matrix4();
    m.compose(new THREE.Vector3(x, 0, z), new THREE.Quaternion().setFromAxisAngle(_yAxis, rotY), new THREE.Vector3(1, 1, 1));
    return m;
}
function wheelMatrix(carX, carZ, rotY, localX, localZ) {
    ...
    const m = new THREE.Matrix4();
    m.compose(new THREE.Vector3(wx, 0.36, wz), new THREE.Quaternion().setFromAxisAngle(_yAxis, rotY), new THREE.Vector3(1, 1, 1));
    return m;
}
...
export function buildParkedCars(scene, ways, nodeMap, proj, colliders, skip) {
    ...
    for (const way of ways) {
        ...
        for (let i = 0; i < pts.length - 1; i++) {
            ...
            while (debt <= L) {
                ...
                if ((aInt && debt < CORNER) || (bInt && L - debt < CORNER)) {
                    debt += SPACING_BASE;
                    continue;
                }
                for (const side of [+1, -1]) {
                    ...
                    if (skip && skip(carX, carZ))
                        continue; // keep cars off the tram tracks
    ...
    const chassisMesh = new THREE.InstancedMesh(CHASSIS_GEO, new THREE.MeshLambertMaterial({ vertexColors: false }), carMats.length);
    chassisMesh.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(carMats.length * 3), 3);
    ...
    const cabinMesh = new THREE.InstancedMesh(CABIN_GEO, new THREE.MeshLambertMaterial({ vertexColors: false }), carMats.length);
    cabinMesh.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(carMats.length * 3), 3);
    for (let i = 0; i < carMats.length; i++) {
    ...
    for (let i = 0; i < carMats.length; i++)
        windowMesh.setMatrixAt(i, carMats[i]);
    windowMesh.instanceMatrix.needsUpdate = true;
    scene.add(windowMesh);
    const wheelMesh = new THREE.InstancedMesh(WHEEL_GEO, MAT_WHEEL, wheelMats.length);
    for (let i = 0; i < wheelMats.length; i++)
        wheelMesh.setMatrixAt(i, wheelMats[i]);
    ...
    const explode = (idx) => {
        ...
        for (let w = 0; w < 4; w++)
            wheelMesh.setMatrixAt(idx * 4 + w, ZERO_MAT);
        chassisMesh.instanceMatrix.needsUpdate = true;
        cabinMesh.instanceMatrix.needsUpdate = true;
        windowMesh.instanceMatrix.needsUpdate = true;
        wheelMesh.instanceMatrix.needsUpdate = true;
