...
function smooth(h: Float32Array, size: number, passes = 2): void {
...
function grainCanvas(size: number, base: [number, number, number], speckle: number, blotch: number): HTMLCanvasElement {
...
function normalCanvas(size: number, strength: number, rough = 1): HTMLCanvasElement {
...
function tex(cv: HTMLCanvasElement, srgb = false): THREE.CanvasTexture {
...
export interface SurfaceMaps { map: THREE.Texture; normalMap: THREE.Texture; }

let _asphalt: SurfaceMaps | null = null;
...
let _concrete: SurfaceMaps | null = null;
export function concreteMaps(): SurfaceMaps {
  ...
  const ctx = cv.getContext('2d')!;
  ctx.strokeStyle = 'rgba(120,116,108,0.5)'; ctx.lineWidth = 2;
...
let _ground: SurfaceMaps | null = null;
export function groundMaps(): SurfaceMaps {
