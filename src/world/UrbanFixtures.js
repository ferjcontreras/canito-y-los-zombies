...
const nameNS = (x) => NS_NAMES[Math.round(x / 100) * 100] ?? '';
const nameEW = (x, z) => {
    const zk = Math.round(z / 100) * 100;
    if (x >= 400)
        return EW_EAST[zk] ?? ''; // Salta ↔ San Martín
    if (x >= -400)
        return EW_MID[zk] ?? EW_PLACE[zk] ?? ''; // San Martín ↔ Belgrano
...
export function buildUrbanFixtures(scene, ways, nodeMap, proj) {
    ...
    for (const w of ways)
        for (const nid of w.nodes) {
            if ((usage.get(nid) ?? 0) < 2 || seen.has(nid))
                continue;
            seen.add(nid);
...
function buildAcequias(scene, ways, nodeMap, proj, usage) {
    const water = [];
    const widx = [];
    const CORNER = 8;
    const Y = 0.088;
    const pushQuad = (out, idx, ax, az, bx, bz, nx, nz, hw) => {
        const base = out.length / 3;
        out.push(ax + nx * hw, Y, az + nz * hw, ax - nx * hw, Y, az - nz * hw, bx + nx * hw, Y, bz + nz * hw, bx - nx * hw, Y, bz - nz * hw);
...
// ── Semáforos ────────────────────────────────────────────────────────────────
function buildTrafficLights(scene, inters) {
    ...
    for (const it of inters) {
        ...
        for (const [sx, sz] of [[1, 1], [-1, -1]]) {
            ...
            m.compose(new THREE.Vector3(it.x + sx * co, 0, it.z + sz * co), new THREE.Quaternion().setFromAxisAngle(yAxis, rotY), new THREE.Vector3(1, 1, 1));
            mats.push(m);
        }
    }
    const inst = (geo, mat) => {
...
// ── Carteles de calles ───────────────────────────────────────────────────────
function buildStreetSigns(scene, inters) {
    const poleMats = [];
    ...
    if (poleMats.length) {
        ...
        mesh.castShadow = true;
        for (let i = 0; i < poleMats.length; i++)
            mesh.setMatrixAt(i, poleMats[i]);
        mesh.instanceMatrix.needsUpdate = true;
        scene.add(mesh);
    }
