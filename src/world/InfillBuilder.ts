...
const HW_FOR_INFILL: Record<string, number> = {
...
class SpatialGrid {
  ...
  overlapsAABB(test: AABB): boolean {
      ...
      for (let iz = iz0; iz <= iz1; iz++) {
        ...
        for (const ab of bucket) {
          if (ab.minX < test.maxX && ab.maxX > test.minX &&
              ab.minZ < test.maxZ && ab.maxZ > test.minZ) return true;
        }
      }
    }
    return false;
...
function makeGabledRoof(
  cx: number, cz: number, w: number, d: number, wallH: number, ang: number,
): THREE.BufferGeometry {
...
// ── Main infill builder ──────────────────────────────────────────────────
export function buildInfillHouses(
  scene: THREE.Scene,
  streets: OSMWay[],
  nodeMap: Map<number, OSMNode>,
  proj: Projection,
  existingColliders: AABB[],
  extraNoBuild: AABB[] = [],
): AABB[] {
  const grid = new SpatialGrid(20);
  for (const ab of existingColliders) grid.insert(ab);
  // No-build zones for roads (so infill never falls on asphalt)
  for (const ab of roadNoBuildZones(streets, nodeMap, proj)) grid.insert(ab);
  // Caller-supplied no-build zones (avenue corridor, plaza, etc.)
  for (const ab of extraNoBuild) grid.insert(ab);

  const wallsByColor:   Map<number, THREE.BufferGeometry[]> = new Map();
  const roofsByColor:   Map<number, THREE.BufferGeometry[]> = new Map();
  const parapetByColor: Map<number, THREE.BufferGeometry[]> = new Map();
  const glassGeos: THREE.BufferGeometry[] = [];
  const litGeos:   THREE.BufferGeometry[] = [];
  const newColliders: AABB[] = [];

  // Window grids on the four façades, split into plain glass and a few lit ones.
  const addWindows = (
    cx: number, cz: number, ang: number,
    W: number, D: number, wallH: number, floors: number, litProb: number,
  ): void => {
    const floorH = wallH / floors;
    const wh = Math.min(1.6, floorH * 0.55);
    const T = 0.1;
    const c = Math.cos(ang), s = Math.sin(ang);
    const l2w = (lx: number, lz: number): [number, number] => [cx + lx * c + lz * s, cz - lx * s + lz * c];
    const halfW = W / 2, halfD = D / 2;

    const faces: Array<{ alongX: boolean; perp: number; len: number }> = [
  ...
  const inCivitHouseZone = (cx: number, cz: number): boolean =>
    cx < -430 && cx > -890 && Math.abs(cz) < 70;

  let nPlaced = 0;

  for (const way of streets) {
    const type = way.tags.highway;
    if (!type || !INFILL_ROADS.has(type)) continue;
    if (way.tags.name && INFILL_SKIP_NAMES.has(way.tags.name)) continue;
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      ...
      while (debt <= L) {
        ...
        for (const side of [+1, -1]) {
          if (Math.random() > HOUSE_PROB) continue;
          ...
          const tryPlace = (baseOff: number, allowHouse: boolean): number | null => {
            ...
            let houseW: number, houseD: number, floors: number, wallH: number;
            if (kind === 'house') {
            ...
            for (const [lx, lz] of [[+halfW, +halfD], [-halfW, +halfD], [+halfW, -halfD], [-halfW, -halfD]] as [number, number][]) {
              const c = Math.cos(ang), s = Math.sin(ang);
            ...
            if (grid.overlapsAABB(test)) return null;

            // ── Walls ──────────────────────────────────────────────────────
            const wallGeo = makeBox(cx, cz, houseW, houseD, wallH, 0, ang);
            if (!wallsByColor.has(wallColor)) wallsByColor.set(wallColor, []);
            ...
            if (isHouse) {
              ...
              if (!roofsByColor.has(roofColor)) roofsByColor.set(roofColor, []);
              ...
              if (!parapetByColor.has(pColor)) parapetByColor.set(pColor, []);
              const edges = parapetByColor.get(pColor)!;
              const ph = 0.45, pw = 0.22;
              const c = Math.cos(ang), s = Math.sin(ang);
              const l2w = (lx: number, lz: number): [number, number] =>
                [cx + lx * c + lz * s, cz - lx * s + lz * c];
              {
            ...
            const collider: AABB = { minX: mnX, maxX: mxX, minZ: mnZ, maxZ: mxZ };
          ...
          for (const prob of BACKROW_PROBS) {
            if (far === null || Math.random() > prob) break;
  ...
  // Commit meshes
  const commit = (
    byColor: Map<number, THREE.BufferGeometry[]>,
    castShadow = true,
  ) => {
  ...
  const commitFlat = (geos: THREE.BufferGeometry[], mat: THREE.Material) => {
  ...
  if (typeof console !== 'undefined') console.log(`Infill: ${nPlaced} houses placed`);
