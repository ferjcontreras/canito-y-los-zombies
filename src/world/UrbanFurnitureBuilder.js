...
export function buildStreetLampsAlongRoads(scene, ways, nodeMap, proj) {
    ...
    for (const way of ways) {
        ...
        if (!type || !LAMP_ROAD_TYPES.has(type))
            continue;
        ...
        for (let i = 0; i < pts.length - 1; i++) {
            ...
            while (debt <= len) {
                if (!((aInt && debt < CORNER) || (bInt && len - debt < CORNER))) {
                    ...
                    if (placeRight)
                        matrices.push(mat4(lx + nx * hw, lz + nz * hw, armRight));
                    if (placeLeft)
                        matrices.push(mat4(lx - nx * hw, lz - nz * hw, armLeft));
                }
                debt += SPACING;
            }
            debt -= len;
...
export function buildStreetLamps(scene, nodes, proj) {
...
// ── Benches (OSM tagged) ───────────────────────────────────────────────────
export function buildBenches(scene, nodes, proj) {
    const matrices = nodes.map(n => {
        const [x, z] = proj.project(n.lat, n.lon);
        return mat4(x, z, Math.random() * Math.PI * 2);
    });
...
export function buildBenchesAt(scene, positions) {
