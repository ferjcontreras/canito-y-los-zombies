...
// ── Marcas / rubros de la calle comercial (Av. San Martín) ─────────────────
// bg = color del cartel/toldo, fg = color del texto.
interface Shop { name: string; bg: number; fg: number; }
const SHOPS: Shop[] = [
...
const _signTex = new Map<string, THREE.CanvasTexture>();
...
function box(cx: number, cz: number, w: number, d: number, h: number, yBase: number): THREE.BufferGeometry {
 ...
 */
export function buildCommercialStrip(
  scene: THREE.Scene,
  xLine: number,
  zStart: number,
  zEnd: number,
  ...
  const wallsByColor = new Map<number, THREE.BufferGeometry[]>();
  const awningByColor = new Map<number, THREE.BufferGeometry[]>();
  const glassGeos: THREE.BufferGeometry[] = [];
  const signMeshes: THREE.Mesh[] = [];
  const newColliders: AABB[] = [];

  const overlaps = (t: AABB): boolean => {
    for (const c of existing)
      if (c.minX < t.maxX && c.maxX > t.minX && c.minZ < t.maxZ && c.maxZ > t.minZ) return true;
  ...
  while (z < zEnd) {
    ...
    for (const side of [1, -1] as const) {
      ...
      if (!awningByColor.has(shop.bg)) awningByColor.set(shop.bg, []);
      awningByColor.get(shop.bg)!.push(
        box(wallX - side * 1.0, cz, 2.0, w * 0.94, 0.22, 2.6),
      );
  ...
  // ── Commit ─────────────────────────────────────────────────────────────
  const commit = (byColor: Map<number, THREE.BufferGeometry[]>, mk: (c: number) => THREE.Material) => {
  ...
  if (typeof console !== 'undefined') console.log(`Comercios San Martín: ${newColliders.length} locales`);
  return newColliders;
}
