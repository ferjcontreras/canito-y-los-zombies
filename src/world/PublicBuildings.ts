...
function signTexture(text: string, bg = '#10307a', fg = '#ffffff'): THREE.Texture {
  const cw = 512, ch = 96;
...
type G = THREE.Group;
const mk = (parent: G, geo: THREE.BufferGeometry, mat: THREE.Material, x: number, y: number, z: number): THREE.Mesh => {
...
function flag(parent: G, x: number, z: number, h = 7): void {
...
function facadeSign(parent: G, text: string, y: number, z: number, w = 8, bg?: string, fg?: string): void {
...
function windowRow(parent: G, W: number, z: number, y: number, n: number): void {
  for (let i = 0; i < n; i++) {
...
function placeGroup(scene: THREE.Scene, x: number, z: number, colliders: AABB[], noBuild: AABB[], half: number): G {
...
export function buildPublicBuildings(scene: THREE.Scene, colliders: AABB[], noBuild: AABB[]): void {
  // ── Casa de Gobierno (columnas + cúpula) ──────────────────────────────────
  {
    ...
    windowRow(g, 22, 8.05, 5, 7); windowRow(g, 22, 8.05, 9, 7);
    for (let i = 0; i < 7; i++) mk(g, new THREE.CylinderGeometry(0.55, 0.6, 9, 12), MARBLE, -10 + i * 10 / 3, 4.5, 8.6);
  ...
  // ── Municipalidad de Mendoza (torre con reloj) ────────────────────────────
  {
    ...
    windowRow(g, 20, 7.55, 4.5, 6); windowRow(g, 20, 7.55, 8, 6);
    // Torre del reloj
  ...
  // ── Escuela (con bandera y patio) ─────────────────────────────────────────
  {
    ...
    windowRow(g, 22, 6.05, 3.2, 7); windowRow(g, 22, 6.05, 6.0, 7);
