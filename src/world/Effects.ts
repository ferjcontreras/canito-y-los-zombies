...
export class EffectsManager {
  ...
  private _flashes: Array<{ light: THREE.PointLight; life: number; max: number; base: number }> = [];
  private _rings: Array<{
    mesh: THREE.Mesh; mat: THREE.MeshBasicMaterial; life: number; max: number; radius: number;
  }> = [];

  constructor(scene: THREE.Scene) { this._scene = scene; }

  // Small spurt for "Canito got hit" — fewer, smaller particles, lower arc
  spawnHurtBlood(pos: THREE.Vector3): void {
    const COUNT = 7;
  ...
  spawnBlood(pos: THREE.Vector3): void {
    const COUNT = 16;
    for (let i = 0; i < COUNT; i++) {
  ...
  spawnExplosion(pos: THREE.Vector3): void {
    ...
    for (let i = 0; i < FIRE_COUNT; i++) {
      ...
      this._particles.push({
        mesh: m,
        vel: new THREE.Vector3(
          Math.cos(ang) * Math.cos(phi) * speed,
          Math.sin(phi) * speed + 4,
          Math.sin(ang) * Math.cos(phi) * speed,
        ),
