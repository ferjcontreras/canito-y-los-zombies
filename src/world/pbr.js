...
export function upgradeToPBR(root) {
    const cache = new Map();
    const convert = (m) => {
        const lm = m;
        if (lm.isMeshStandardMaterial || lm.isMeshBasicMaterial || lm.isShaderMaterial)
            return m;
        if (!lm.isMeshLambertMaterial && !lm.isMeshPhongMaterial)
            return m;
