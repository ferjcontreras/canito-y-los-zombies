...
function pickColor(palette: number[], id: number): number {
  return palette[Math.abs(id) % palette.length];
}

function paletteFor(tags: Record<string, string>): number[] {
  const t = tags.building;
  if (!t) return PAL_DEFAULT;
  if (t === 'church' || t === 'cathedral' || t === 'chapel' || t === 'temple') return PAL_RELIGIOUS;
  if (t === 'industrial' || t === 'warehouse' || t === 'factory') return PAL_INDUSTRIAL;
  if (t === 'commercial' || t === 'retail' || t === 'office' || t === 'hotel') return PAL_COMMERCIAL;
  if (t === 'house' || t === 'detached' || t === 'semidetached_house' || t === 'bungalow') return PAL_HOUSE;
  if (t === 'residential' || t === 'apartments') return PAL_RESIDENTIAL;
  return PAL_DEFAULT;
}
...
function buildingHeight(tags: Record<string, string>): number {
  ...
  const t = tags.building;
  if (t === 'apartments')                                    return 5 * 3.2;
  if (t === 'house' || t === 'detached' || t === 'semidetached_house') return 2 * 3.2;
  if (t === 'residential')                                   return 4 * 3.2;
  if (t === 'commercial' || t === 'retail')                  return 3 * 3.2;
  if (t === 'office' || t === 'hotel')                       return 6 * 3.2;
  if (t === 'industrial' || t === 'warehouse')               return 2 * 3.2;
  if (t === 'church' || t === 'cathedral')                   return 10 * 3.2;
  return 3 * 3.2;
}

function wantsGabledRoof(tags: Record<string, string>): boolean {
  const t = tags.building;
  if (!t) return false;
  return (
    t === 'house' || t === 'detached' || t === 'semidetached_house' ||
...
// ── AABB from ring ─────────────────────────────────────────────────────────
function ringAABB(ring: [number, number][]): AABB {
  let minX = Infinity, maxX = -Infinity, minZ = Infinity, maxZ = -Infinity;
  for (const [x, z] of ring) {
    if (x < minX) minX = x; if (x > maxX) maxX = x;
    if (z < minZ) minZ = z; if (z > maxZ) maxZ = z;
...
export function buildBuildings(
  ...
  const wallGeos:    Map<number, THREE.BufferGeometry[]> = new Map();
  const parapetGeos: Map<number, THREE.BufferGeometry[]> = new Map();
  const roofGeos:    Map<number, THREE.BufferGeometry[]> = new Map();

  const colliders: AABB[] = [];

  const addTo = (map: Map<number, THREE.BufferGeometry[]>, color: number, geo: THREE.BufferGeometry) => {
