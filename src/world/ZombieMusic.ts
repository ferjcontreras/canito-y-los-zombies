...
interface Chord { bass: number; pad: number[]; }
...
const THEME: Array<{ d: number | null; b: number }> = [
  // "Men-do-za"
...
export class ZombieMusic {
  ...
  private musicGain!: GainNode;      // master de música (toggle con M)
  private synthBus!: GainNode;       // mezcla de la música procedural
  private trackGain!: GainNode;      // pista real (si existe music.mp3)
  private trackLP!: BiquadFilterNode;
  private delay!: DelayNode;
  private noiseBuf!: AudioBuffer;
  private trackLoaded = false;
  private readonly trackUrl = 'audio/music.mp3';
  ...
  private stepDur = 0;          // duración de una semicorchea
  private nextTime = 0;
  private step = 0;
  private bar = 0;
  private melIdx = 0;
  private melTime = 0;
  private intensity = 0;        // 0..1, lo sube el juego en combate
  ...
  unlock(): void {
    ...
    this.trackLP.type = 'lowpass'; this.trackLP.frequency.value = 1600;
  ...
  private async _tryLoadTrack(): Promise<void> {
    const ctx = this.ctx!;
    try {
  ...
  setEnabled(on: boolean): void {
    this.enabled = on;
    if (this.ctx) this.musicGain.gain.setTargetAtTime(on ? 0.5 : 0.0, this.ctx.currentTime, 0.2);
  }
  ...
  setIntensity(v: number): void { this.intensityTarget = Math.max(0, Math.min(1, v)); }
  ...
  private _melody(t: number, freq: number, dur: number): void {
    const ctx = this.ctx!;
    const o1 = ctx.createOscillator(); o1.type = 'triangle'; o1.frequency.value = freq;
    const o2 = ctx.createOscillator(); o2.type = 'sine'; o2.frequency.value = freq; o2.detune.value = -7;
    // vibrato
    ...
    const lp = ctx.createBiquadFilter();
    lp.type = 'lowpass'; lp.frequency.value = 1100 + this.intensity * 1900;
  ...
  private _step(step: number, chordIdx: number, t: number): void {
    ...
    if (step === 4 || step === 12) this._noise(t, 0.12 + I * 0.22, 0.18, 1200);
    // Hi-hats en las semicorcheas impares cuando hay tensión
    if (I > 0.35 && step % 2 === 1) this._noise(t, 0.04 + I * 0.06, 0.05, 6000);
    ...
    if (step === 0) this._pad(t, ch.pad, (60 / this.BPM) * 4 * 0.95);
  ...
  private _bass(t: number, freq: number, level: number): void {
    const ctx = this.ctx!;
    const o = ctx.createOscillator(); o.type = 'sawtooth'; o.frequency.value = freq;
    const lp = ctx.createBiquadFilter();
    lp.type = 'lowpass'; lp.frequency.value = 380 + this.intensity * 900; lp.Q.value = 6;
    const g = ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
  ...
  private _noise(t: number, level: number, dur: number, hp: number): void {
    const ctx = this.ctx!;
    const src = ctx.createBufferSource(); src.buffer = this.noiseBuf;
    src.loop = true; src.playbackRate.value = 1 + Math.random() * 0.3;
  ...
  private _pad(t: number, freqs: number[], dur: number): void {
    const ctx = this.ctx!;
    const lp = ctx.createBiquadFilter();
    lp.type = 'lowpass'; lp.frequency.value = 900 + this.intensity * 700;
    const g = ctx.createGain();
    g.gain.setValueAtTime(0.0001, t);
  ...
  private _lead(t: number, freq: number, level: number): void {
    const ctx = this.ctx!;
