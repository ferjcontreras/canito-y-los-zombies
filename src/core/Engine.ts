...
export class Engine {
  ...
  private _bloom: UnrealBloomPass | null = null;
  private _gtao: GTAOPass | null = null;
  private _smaa: SMAAPass | null = null;

  // Luces (para el ciclo de atardecer → noche)
  private _sun!: THREE.DirectionalLight;
  private _ambient!: THREE.AmbientLight;
  private _hemi!: THREE.HemisphereLight;

  // Calidad de render: 0 = Rendimiento, 1 = Equilibrada (def.), 2 = Alta.
  private _quality = 1;
  private static readonly QUALITY_NAMES = ['Rendimiento', 'Equilibrada', 'Alta'];
  ...
  private static readonly _SKY_GOLD  = new THREE.Color(0xf2b277);
  private static readonly _SKY_ORANGE = new THREE.Color(0xd9622e);
  private static readonly _SKY_NIGHT = new THREE.Color(0x0a0e1a);
  private static readonly _SUN_DAY   = new THREE.Color(0xffb265);
  private static readonly _SUN_NIGHT = new THREE.Color(0x35304e);
  private readonly _tmpSky = new THREE.Color();
  private readonly _tmpSun = new THREE.Color();
  ...
  setDusk(t: number): void {
    ...
    if (k < 0.5) this._tmpSky.copy(Engine._SKY_GOLD).lerp(Engine._SKY_ORANGE, k / 0.5);
    else         this._tmpSky.copy(Engine._SKY_ORANGE).lerp(Engine._SKY_NIGHT, (k - 0.5) / 0.5);
    (this.scene.background as THREE.Color).copy(this._tmpSky);
    (this.scene.fog as THREE.FogExp2).color.copy(this._tmpSky);
  ...
  private _buildComposer(cam: THREE.Camera): void {
  ...
  get qualityName(): string { return Engine.QUALITY_NAMES[this._quality]; }
  ...
  setQuality(level: number): void {
    ...
    if (this._gtao)  this._gtao.enabled  = q >= 2;   // GTAO sólo en Alta (es lo más caro)
    if (this._bloom) this._bloom.enabled = q >= 1;
    if (this._smaa)  this._smaa.enabled  = q >= 1;
  ...
  cycleQuality(): void { this.setQuality(this._quality + 1); }
  ...
  start(onFrame: (dt: number) => void): void {
    const loop = (now: number) => {
      ...
      else if (this._camera) this.renderer.render(this.scene, this._camera);
