...
export class FlyCamera {
  ...
  private pitch = -0.3;  // radians, slightly downward
  private yaw   = 0;     // radians, facing south (+Z)
  private pointerLocked = false;

  private readonly BASE_SPEED  = 40;   // m/s
  private readonly TURBO_MULT  = 5;
  private readonly MOUSE_SENS  = 0.0018;
  ...
  private _bindPointerLock(canvas: HTMLElement): void {
    canvas.addEventListener('click', () => canvas.requestPointerLock());

    document.addEventListener('pointerlockchange', () => {
      this.pointerLocked = document.pointerLockElement === canvas;
      const hint = document.getElementById('pointer-hint');
      if (hint) hint.classList.toggle('hidden', this.pointerLocked);
    });
  ...
  update(dt: number): void {
    ...
    if (this.keys.has('Space'))                                move.addScaledVector(up,       1);
    if (this.keys.has('ShiftLeft') || this.keys.has('ShiftRight')) move.addScaledVector(up, -1);
