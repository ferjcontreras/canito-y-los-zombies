import type { BBox, OSMData } from './types';
...
function readCache(key: string): OSMData | null {
  try {
    const raw = localStorage.getItem(key);
    if (!raw) return null;
    const { data, ts } = JSON.parse(raw) as { data: OSMData; ts: number };
    if (Date.now() - ts > CACHE_TTL_MS) return null;
    return data;
  } catch { return null; }
}

function writeCache(key: string, data: OSMData): void {
...
export async function fetchCity(
  ...
  if (cached) { onProgress?.('Cargando desde caché…'); return cached; }
  ...
  if (!res.ok) throw new Error(`Overpass HTTP ${res.status}. Reintentá en unos minutos.`);
